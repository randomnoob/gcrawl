from gcrawl_sockspin_core import pxssh
import traceback
import subprocess
import requests
import time
    ...
    def check_ifsocket(self, path, tries=3, wait=0):
        try:
            ...
            if tries==0:
                ...
                print ('Wait 1 sec and check SOCKET_PATH again')
    ...
    def forkspin(self, host, username, password, port):
        ...
        try:
            ...
            if login_attempt:
                ...
                socket_path = os.path.join(self.cwd, 'socket/{}.sock'.format(port))
                # SSHPASS comes to the rescue
                cmd = SSHPASS_CMD_TEMPLATE.format(username=username, password=password, port=port, host=host, socket=socket_path)
                print ('____________________________________________________________________________\n{}'.format(cmd))
                # Spawn SSHPASS
                p = subprocess.Popen(cmd, shell=True)
                print ("Spawned 127.0.0.1:{}".format(port))
                # Checks if socket_path actually exists
                # Wait 1 seconds for the socket to initialize
                time.sleep(1)
                if self.check_ifsocket(socket_path):
...
#             logfile=open("/home/nl/dev/sshconn/mylog.txt", "w")
#             # Socket path set to current working dir
#             socket_path = os.path.join(os.getcwd(), 'socket/{}.sock'.format(port))
#             # SSHPASS comes to the rescue
#             cmd_template= 'sshpass -p {password} ssh -D {port} -M -S {socket} -f -q -N -C {username}@{host}'
#             cmd = cmd_template.format(username=username, password=password, port=port, host=host, socket=socket_path)
#             print ('____________________________________________________________________________')
#             print (cmd)
#             # Spawn SSHPASS
#             p = subprocess.Popen(cmd, shell=True)
