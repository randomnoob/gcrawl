...
DATA_INPUT_DIR = 'download'
DATA_OUTPUT_DIR = '{}_converted'.format(DATA_INPUT_DIR)
...
class Post:
    def __init__(self, title):
        ...
        self.meta = ''
        self.content = ''
        self.content_skeleton = '![{image_alt}]({image_url} "{image_alt}")\n\n  ©Image courtesy of {image_refername}. [Original Page : {image_referhomepage}]({image_referurl})\n\n'
        self.footer = ''
        self.num_images = 0
    ...
    def insert_contentblock(self, image_meta_dict):
        if self.num_images < 50:
            ...
            if not self.meta:
                self.meta = 'Title:{}\nDate:2018-12-22\nAuthor:MedDB\nTags:General\nSummary:{}\nSlug:{}\n\n'\
                            .format(self.title, skeleton_dict['image_alt'], self.slug)
            self.content += block
            self.num_images += 1
    ...
    def save(self, filename, path):
        ...
        with open(fullpath, 'w', encoding='utf-8') as output:
            output.write(self.meta + self.content + self.footer)
        # print ('Saved {} contains {} blocks'.format(filename, self.num_images), end='\r')



class CoreProcess:
    def __init__(self, output_path=DATA_OUTPUT_DIR, input_path=DATA_INPUT_DIR):
        """
        ...
        if not os.path.exists('./{}'.format(self.output_path)):
            os.mkdir('./{}'.format(self.output_path))
    # -------------------------------
